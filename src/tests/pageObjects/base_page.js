...
export default class BasePage {
  ...
  static async waitUntil(
    conditionCallback,
    timeoutMsg = 'Condition not met in time',
  ...
  static async waitForAllElementsToIncludeText(
    elementsGetter,
    expectedText,
    timeoutMsg = 'Not all elements include expected text',
  ...
  static async waitUntilClickable(element, timeout = BasePage.timeout, timeoutMsg = '') {
    await element.waitForDisplayed({
      timeout,
      timeoutMsg: timeoutMsg || `Element not displayed after ${timeout}ms`,
    });
    await element.waitForEnabled({
      timeout,
      timeoutMsg: timeoutMsg || `Element not enabled after ${timeout}ms`,
    });
    await element.waitForClickable({
      timeout,
  ...
  static async waitUntilUrlContains(
    expectedPart,
    timeout = this.timeout,
    timeoutMsg = `Expected URL to contain ${expectedPart}`
  ) {
    return await browser.waitUntil(async () => (await browser.getUrl()).includes(expectedPart), {
      timeout,
      timeoutMsg,
    });
  }
